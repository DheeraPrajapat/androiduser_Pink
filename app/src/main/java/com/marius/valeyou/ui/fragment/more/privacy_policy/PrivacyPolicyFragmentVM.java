...
public class PrivacyPolicyFragmentVM extends BaseFragmentViewModel {
    ...
    final SingleRequestEvent<PrivacyResponseBean> termsEvent = new SingleRequestEvent<>();

    @Inject
    public PrivacyPolicyFragmentVM(SharedPref sharedPref, NetworkErrorHandler networkErrorHandler, WelcomeRepo welcomeRepo) {
    ...
    public void getDetailsTerms(int type) {
        welcomeRepo.getAllContent(sharedPref.getUserData().getAuthKey(),type).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<ApiResponse<PrivacyResponseBean>>() {
            @Override
            public void onSubscribe(Disposable d) {
                compositeDisposable.add(d);
                termsEvent.setValue(Resource.loading(null));
            }

            @Override
            public void onSuccess(ApiResponse<PrivacyResponseBean> apiResponse) {
                if (apiResponse.getStatus() == HttpURLConnection.HTTP_OK) {
                    termsEvent.setValue(Resource.success(apiResponse.getData(),apiResponse.getMsg()));
                } else {
                    termsEvent.setValue(Resource.warn(null,apiResponse.getMsg()));
                }
            }

            @Override
            public void onError(Throwable e) {
                termsEvent.setValue(Resource.error(null,networkErrorHandler.getErrMsg(e)));
            }
        });
    }
